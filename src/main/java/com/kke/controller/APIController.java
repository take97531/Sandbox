...
public class APIController {
	...
	public ResponseEntity<Integer> phoneId(@Validated @RequestBody ApiVO api, UriComponentsBuilder uriBuilder) throws Exception {
		...
		if(service.apiChk(api) == 0)
			return new ResponseEntity<>(1, HttpStatus.OK);
		
		if(service.phoneChk(api) != 0)
			return new ResponseEntity<>(1, HttpStatus.OK);
		
		service.phoneId(api);
		//service.IdRegister(api);
		
		System.out.println("register member.getuserRRN() = " + api.getUser_RRN());
		
		
		URI resourceUri = uriBuilder.path("apis/phoneId/{userNum}")
	...
	public ResponseEntity<Integer> phoneChk(@Validated @RequestBody ApiVO api) throws Exception {
		...
		int result = service.phoneChk(api);
		System.out.println("result = " + result);
		
		if(result == 1) {
			int sum = 0;
			List<ApiVO> accList = service.getAccNum(api);
			...
			for(int i =0; i<accList.size(); i++) {
				...
				int amount = service.getWithdraw(accList.get(i));
	...
	// 전월 지출정보 response/request
		...
		public ResponseEntity<List<ApiVO>> accountInfo(@Validated @RequestBody ApiVO api) throws Exception {
			...
			if(result == 1) {
				...
				for(int i=0; i<accList.size(); i++) 
					boardList.addAll(service.showApiList(accList.get(i)));
